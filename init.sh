...
# Start paxos node.
${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=0 -proxy=0 2> /dev/null &
PAXOS_NODE_PID0=$!
sleep 1

${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=1 -proxy=0 2> /dev/null &
PAXOS_NODE_PID1=$!
sleep 1

${PAXOS_NODE} -ports=${ALL_PORTS} -N=3 -id=2 -proxy=0 2> /dev/null &
PAXOS_NODE_PID2=$!
sleep 1
