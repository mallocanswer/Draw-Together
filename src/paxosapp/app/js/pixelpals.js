$(document).ready(function() {
    ...
    PixelCanvas.prototype.load = function (data) {
        for (var r = 0; r < this.rows; ++r) {
            for (var c = 0; c < this.cols; ++c) {
                this.setColor( data[r*this.cols + c] );
                this.setPixel(c, r);
            }
        }
    }
    ...
    PixelCanvas.prototype.setNetworkPixel = function (x, y) {
        ...
        key = y*this.cols + x;
        $.ajax({
            url: 'setpixel?key='+key+'&color='+this.color.substring(1),
            async: false
        });
    }
    ...
    Picker.prototype.onMouseUp = function (event) {
        if (this.drawing) {
            ...
            if (this.dragging) { // No mouse up action if mouse was being dragged
                ...
                var point = this.pixelCanvas.getPixelpoint(event);
                var color = this.pixelCanvas.getPixel(point.x, point.y);
    ...
    function Ui () {
        ...
        $('#eraser').click($.proxy(function (event) {
    ...
    var ui = new Ui();
    (function getPixels() {
        $.getJSON('getpixels', function(data) {
            rows = ui.w / ui.pixelSize;
            cols = ui.h / ui.pixelSize;
            color = ui.pixelCanvas.color;
            for (var r = 0; r < rows; ++r) {
                for (var c = 0; c < cols; ++c) {
                    ui.pixelCanvas.setDataPixel(r*cols+c, '#'+data[r*cols+c]);
                }
            }
            ui.pixelCanvas.color = color;
